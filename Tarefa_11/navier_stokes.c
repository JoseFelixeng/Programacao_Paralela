/*
 * navier_stokes_viscoso.c
 * -----------------------------------------------------------------------
 * Simulação simplificada da equação de Navier-Stokes considerando APENAS
 * o termo de viscosidade (difusão de momento). Termos desconsiderados:
 *   - Pressão (∇p)
 *   - Forças externas (f)
 *   - Advecção não-linear ((u·∇)u)  -> mantida de fora por simplicidade,
 *     já que o enunciado pede só o efeito da viscosidade.
 *
 * Equação resolvida (para cada componente de velocidade u e v):
 *
 *      du/dt = nu * (d²u/dx² + d²u/dy²)
 *      dv/dt = nu * (d²v/dx² + d²v/dy²)
 *
 * que é exatamente a equação do calor/difusão aplicada a cada componente
 * do campo de velocidade. É discretizada no espaço com diferenças finitas
 * centradas de segunda ordem (5 pontos) em uma malha 2D uniforme, e no
 * tempo com um esquema explícito (Euler para frente / FTCS).
 *
 * Condição de estabilidade do esquema explícito em 2D:
 *      nu * dt * (1/dx² + 1/dy²) <= 0.5
 *
 * O programa faz duas fases:
 *   FASE 1 - Verificação de estabilidade:
 *     Inicializa o fluido parado (u=v=0) ou com velocidade constante e
 *     roda a simulação, mostrando que o campo permanece estável (não
 *     cresce nem oscila), pois a difusão de um campo uniforme não o
 *     altera.
 *
 *   FASE 2 - Perturbação e difusão:
 *     Aplica uma pequena perturbação gaussiana localizada no centro do
 *     domínio (um "pulso" de velocidade) e deixa o sistema evoluir,
 *     observando a perturbação se espalhar (difundir) suavemente pelo
 *     domínio ao longo do tempo, como esperado fisicamente pelo efeito
 *     da viscosidade.
 *
 * Saída:
 *   - Estatísticas (velocidade máxima, velocidade média) impressas no
 *     terminal a cada N passos, para acompanhar a evolução.
 *   - Arquivos .csv com snapshots do campo u(x,y) em instantes escolhidos
 *     da Fase 2, para permitir visualização externa (ex.: Python/gnuplot).
 *
 * Compilação:
...
static void salva_csv(const double *u, int passo) {
    ...
    snprintf(nome_arquivo, sizeof(nome_arquivo), "saida/snapshot_u_passo_%05d.csv", passo);
